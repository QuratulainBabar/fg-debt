...
import { statusLabels, useClientPortal } from "@/lib/client-portal-api";
...
function CasesPage() {
  ...
  return <CasesContent cases={data.portal.cases} />;
}

function CasesContent({ cases }: { cases: import("@/lib/client-portal-api").ClientPortalCase[] }) {
    ...
    <>
      ...
      {cases.length === 0 ? (
        <p className="surface-card p-10 text-center text-sm text-muted-foreground">
          No cases yet. Complete your assessment or wait for your solicitor to open a matter.
        </p>
      ) : (
