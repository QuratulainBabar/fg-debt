...
import { Search, Plus, CreditCard, PoundSterling, Calculator, FileCheck } from "lucide-react";
import { FINANCIAL_RULES } from "@/lib/admin-data";

export const Route = createFileRoute("/_admin/admin/financial-rules")({
  head: () => ({ meta: [{ title: "Financial Rules — FG Debt Advisor AI" }] }),
  component: AdminFinancialRulesPage,
});

function AdminFinancialRulesPage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      <PageHeader
        ...
        description="Manage income/expenditure standards (CFS, SFS), disposable income calculations, and surplus scoring logic."
        actions={
          <Button className="rounded-xl gradient-deep text-primary-foreground shadow-soft">
            <Plus className="size-4 mr-1.5" /> New Rule Set
          </Button>
        }
      />
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        {[
          { label: "Active Rules", value: FINANCIAL_RULES.filter(r => r.status === "active").length, icon: CreditCard, color: "emerald" },
          { label: "Spending Categories", value: 42, icon: PoundSterling, color: "blue" },
          { label: "Surplus Calc Models", value: 4, icon: Calculator, color: "purple" },
          { label: "Pending Review", value: 1, icon: FileCheck, color: "amber" },
        ].map((s) => (
          <Card key={s.label} className="surface-card">
            <CardContent className="p-4 flex items-center gap-3">
              <div className={`grid size-10 place-items-center rounded-xl border ${
                ...
                s.color === "purple" ? "bg-purple-500/10 text-purple-600 border-purple-500/20" :
      ...
      <Card className="surface-card">
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <CardTitle className="text-base font-display">Income & Expenditure Rule Sets</CardTitle>
              <CardDescription className="text-xs">Common Financial Statement (CFS) and Standard Financial Statement (SFS) configurations.</CardDescription>
            </div>
            <div className="relative">
              <Search className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <Input placeholder="Search financial rules…" className="pl-9 h-9 w-56 rounded-xl" />
            </div>
          </div>
        </CardHeader>
